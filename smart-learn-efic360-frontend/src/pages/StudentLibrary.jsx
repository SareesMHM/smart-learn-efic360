...
// Ensure cookies (httpOnly JWT) are sent
...
export default function StudentLibrary() {
  ...
  // ---- NEW: mark all notifications as read on enter (with fallback) ----
  const markAllNotificationsRead = useCallback(async () => {
    try {
      ...
      if (status === 404 || status === 405) {
        // Fallback: fetch unread, then mark-many
        try {
          const res = await axios.get("/api/notifications", {
            params: { unread: true, limit: 500 },
          });
          const ids = (res.data?.data || []).map((n) => n._id);
          if (ids.length) {
            await axios.put("/api/notifications/read-many", { ids });
          }
        } catch (e2) {
          console.error("Fallback mark-many failed:", e2?.response?.data || e2.message);
        }
      } else {
        console.error("mark-all failed:", e?.response?.data || e.message);
      }
    }
  }, []);

  useEffect(() => {
    // Fire-and-forget: mark all as read when entering this page
              ...
              <div className="cm-links">
                ...
                {m.type === "video" &&
                  (m.link ? (
                    ...
                    <video controls width="360" src={m.file.startsWith("http") ? m.file : `/uploads/${m.file}`} />
                  ) : null)}

                {/* PDF */}
                {m.type === "pdf" && m.file && (
                  <a className="btn ghost" href={m.file.startsWith("http") ? m.file : `/uploads/${m.file}`} target="_blank" rel="noreferrer">
                    Open PDF
                  </a>
                )}
                ...
                {isQuiz &&
                  (quizExpired ? (
                    ...
                    <Link className="btn primary" to={`/quiz/${m._id}`}>
                      Take Quiz
                    </Link>
                  ))}
...
function AssignmentSubmitButton({ assignment }) {
  ...
  const submit = async () => {
    ...
    try {
      setSubmitting(true);
      setMsg("");
      await axios.post(`api/assignment/submit`, fd, {
        headers: { "Content-Type": "multipart/form-data" },
